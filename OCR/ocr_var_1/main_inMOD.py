...
global  order # new_coord_line, new_coord_box, spec_general_data, head_data, body_general, image_file, integral_attr
...
if __name__=='__main__':
    ...
    """ 
    감정평가표/감정평가명세표의 좌표+텍스트만 따로 파일로 저장 
    from tqdm import tqdm
    for file in tqdm(os.listdir(image_dir)):
        image_full_path = image_dir + file
        engine(image_full_path)
        new_coord_line, new_coord_box = coord(image_full_path)

        with open("D:/box.txt", 'a', encoding='utf-8') as w:
            w.write(str(image_full_path))
            w.write("\n")
            w.write(str(new_coord_box))
            w.write("-"*100)            
            w.write("\n"*2)
    """ 
    ...
    for idir in os.listdir(image_dir): 
        ...
        for image_file in ddir:
            ...
            HT = HeadTable(idir, image_file, image_full_path, new_coord_line, new_coord_box)
            ...
            for head_title in new_coord_line: # 제목인식은 라인모드로    
                if head_title[0][1] < 400:
                    ...
                    if re.search("[)\]]?\s?감\s?정\s?평\s?가\s?표?$|평\s?가\s?표?$", head_title[5]): # 페이지 상단부에서 찾기 
                        ...
                        # head_data = HT.head_table()
                        # print("이것이 헤드데이터다!!!!", head_data)
                        ...
                        for table_kind in new_coord_line:  #for m3 in new_coord_line:
                            ...
                            if re.search("일련|^지\s?번$", table_kind[5]) and table_kind[0][1] < 700:  ## A 형 감정평가명세표 #if re.search("^소", m3[5]) and m3[0][1] < 600: #if abs(table_kind[0][1] - m3[0][1]) < 50:
                                ...
                                CB.ParseSTA(new_coord_box, new_coord_line)

                                print(key_blocks_A)
                                #spec_general_data, B_list, L_list, for_json_sta = CB.spec_table_A() #idir, image_file, image_full_path, new_coord_line, new_coord_box, next(number)
