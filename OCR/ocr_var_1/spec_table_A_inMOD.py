...
class CompoundBuilding(object): # 집합건물이라는 클래스,,, 무엇을 담을까
    #new_coord_line = []
   # new_coord_box = []

    #num_checked_coord_box= []


    def __init__(self, idir, **kwargs):
    ...
    #def __init__(self, addr, addrnum, dong, chung, ho, price):
    #def __init__(self, idir, image_file, image_full_path, new_coord_line, new_coord_box, number):
    ...
    #    self.price=price   
        ...
        # self.land_attr = {}     # 이 속성들 최대한 간단히 합하기
        # self.building_attr = {}
        # self.general_attr = {}
        # self.B_list = []
        # self.L_list = []
        # self.spec_general_data = []
        # self.spec_A_errors = []
        # self.for_json_sta = {}
        # self.integral_attr={} ## 생략할지??????????????


# class SpecTableA():

#     def __init__(self, new_coord_line, new_coord_box):
#         self.new_coord_line=new_coord_line
#         self.new_coord_box=new_coord_box

        target_js = r"D:/results/forDL/jsons/" # 딥러닝 학습용 json 파일들 저장소


    def ParseSTA(self, new_coord_box, new_coord_line):
        

        def Monitor(self):
            print("모니터링 시작________________________")
            
            print("\n", " "*20, "::: 감정평가명세표 A형 제 {}페이지 분석시작 :::".format(self.page_number), "\n\n")




        def find_keys(self, new_coord_box, new_coord_line):
            ...
            NKL = []
            for ek in KL:
                nek = [ek[0], ek[1], ek[2], ek[3], ek[4], ek[5].replace(" ", "")] #replace로 단어내 모든 공백을 제거
                NKL.append(nek)
            ...
            #kb_list = ["addr", "addrnum", "price"] #minimal 형


            for nk in NKL: # 수집된 키단어가 어떤 것인지 범주화(좌표가 잘 포함되어 있는지 확인바람)
            ...
            #with open ("D:/key_jsons/{}.pkl".format(idir))
                
                

                
            #FUNSD_json = {"form": [{"box":[]key_blocks_A}]}
            #print("FUNSD를 위한 키위치와 텍스트임", FUNSD_json)

            return key_blocks_A, unmet_keys_box_2

        Monitor(self), find_keys(self, new_coord_box, new_coord_line)    
...
"""
        ...
        def vertical_ref_point(self, key_blocks_A, unmet_keys_box_2):
            ### 항목별 격벽의 좌표를 정해둠 ####        
            print("\n", " "*20, ":"*10, "인식된 키박스(A형)::세로기준점", ":"*10, "\n")  
            ...
            ## 미완성키박스에 있는 키들 처리하기
            for unkey in unmet_keys_box_2:
                if unkey == "areause":
                    if 'buildstr' in key_blocks_A.keys():                          
        ...
        def correct_price(self, key_blocks_A, paper_L, price_L, price_R):               
            ...
            return num_checked_coord_box


        def horizontal_ref_point(self, num_checked_coord_box, landclass_L, landclass_R, buildstr_L, buildstr_R, estim_L, estim_R, price_L, price_R, paper_L, paper_R):   
        ...
        def get_general_attr(self, no2_landclass_level, num_checked_coord_box,  num_L, num_R, estim_L, estim_R, addr_L, addr_R, buildstr_R, buildstr_L, memo_Y, addrnum_L, addrnum_R, landclass_L, landclass_R, paper_Y, paper_L, paper_R):
            ...
            if len(b_name_1) > 0:
                ...
                self.general_attr["건물명_1"] = ' '.join(new_b_name_1)
                self.for_json_sta["건물명_1"] = ' '.join(new_b_name_1)
                        
            if len(b_name_2) > 0:
                new_b_name_2 = []
                b_name_2.sort(key=lambda t: t[0][1])
                self.general_attr["건물명_2"] = ' '.join(new_b_name_2)
                ...
                if len(groups) > 0:
                    ...
                    self.general_attr["건물명_bn"] = ' '.join(new_bn_2)        
            ...
            if len(use_mode) > 0:
                ...
                self.general_attr["이용상황"] = ' '.join(new_use_mode)
                self.for_json_sta["이용상황"] = ' '.join(new_use_mode)
            ...
            if len(addr) > 0:
                ...
                self.general_attr["소재지"] = ' '.join(new_addr)
                self.for_json_sta["소재지"] = ' '.join(new_addr)
            ...
            if len(addrnum) > 0:
                ...
                for nadm in addrnum:
                    ...
                    #if 600 < adm[0][1] < no2_landclass_level[0]:
                self.general_attr["지번"] = ' '.join(new_addrnum)
                self.for_json_sta["지번"] = ' '.join(new_addrnum)
                
            # 수집된 각 항목의 리스트를 for 문 종료시 딕셔너리에 부가한다.
            self.integral_attr["평가서 ID"] = self.idir
            ...
            if len(floor_num) > 0:            
                ...
                self.general_attr["지상 층 수"] = int(floor_num[-1])
                self.for_json_sta["지상 층 수"] = int(floor_num[-1])


            if len(self.integral_attr) > 0:
                print("\n", "일반항목", self.general_attr, "\n")

                self.spec_general_data.append(self.integral_attr)
                        ...
                        if abs(midpoint(va)[1] - ue) < 50:
                            ...
                            if  num_L < midpoint(va)[0] < num_R:
                                ...
                                if re.search("\d+\-?\d+", va[5]):
                                    ...
                                    elif "지번" not in self.integral_attr:
                                        self.general_attr["지번"] = "{}".format(va[5])
                                        self.for_json_sta["지번"] = "{}".format(va[5]) 
                                    #print(integral_attr)
            ...
            print(" 토지속성::", self.L_list, "\n")

            #########################################################################################################



        def get_by_price_level(self, no2_daejikwon_level, no2_price_level):
            ...
            if len(no2_daejikwon_level) == 0 and len(no2_price_level) > 0:
                ...
                for p, pr in enumerate(no2_price_level):
                    ...
                    for ind, vn in enumerate(self.num_checked_coord_box):
                        ...
                        if abs(midpoint(vn)[1]- pr) < 50: #  vn은 평가액라인 속으로 좁힘. 
                            ...
                            if price_L < vn[0][0] < price_R:  
                                if re.search("^(\d{1,3}\,)+\d{1,3}$", vn[5]):
                                    self.building_attr["감정평가액"] =  "{}".format(vn[5])
                            ...
                            if landclass_R < midpoint(vn)[0] < buildstr_R:       
                                if re.search("호$", vn[5]):
                                    self.building_attr["호"] =  "{}".format(vn[5])  
                                    self.for_json_sta["호"] = "{}".format(vn[5])

                                if re.search("층$", vn[5]):
                                    self.building_attr["층"] =  "{}".format(vn[5]) 
                                    self.for_json_sta["층"] = "{}".format(vn[5])
                    ...
                    if p_breaker:
                        ...
                        building_attr_copy = self.building_attr.copy()
                        self.B_list.append(building_attr_copy) 
                        
                        self.building_attr.clear()
                ...
                for ind, vn in enumerate(num_checked_coord_box):
                    ...
                    for v_5 in num_checked_coord_box:
                        ...
                        if abs(midpoint(vn)[1]- no2_price_level[0]) < 50: #  vn은 평가액라인 속으로 좁힘.                         
                            ...
                            if num_L < midpoint(vn)[0] < num_R:
                                if re.search("\w{1,3}", vn[5]):
                                    self.integral_attr["일련번호_건물"] = "{}".format(vn[5])   
                                    self.for_json_sta["일련번호_건물"] = "{}".format(vn[5])  
                                    #print("일련번호_건물::", vn[5])
                                ...
                                if abs(midpoint(vn)[1]- thp) < 50: #  vn은 평가액라인 속으로 좁힘. 
                                    ...
                                    if price_L < vn[0][0] < price_R:  
                                        ...
                                        if re.search("호", v_5[5]) and abs(midpoint(v_5)[1] - thp) < 100:
                                            self.building_attr["호"] =  "{}".format(v_5[5])  
                                            self.for_json_sta["호"] = "{}".format(vn[5])

                                        elif re.search("층", v_5[5])and abs(midpoint(v_5)[1] - thp) < 100:
                                            self.building_attr["층"] =  "{}".format(v_5[5]) 
                                            self.for_json_sta["층"] = "{}".format(vn[5])
            ...
            ####################################################################################################
            print("빌딩속성", self.B_list, "\n")

            if len(self.B_list)==0:
                print("건물속성이 모아지지 않았습니다")
                berr_note = "파일명: {}__ 건물속성이 모아지지 않았습니다::".format(self.idir)
                with open(r"D:/results/spec_general_attr_errors.txt", 'a') as f:
                    f.write("\n")
                    f.write(berr_note)


            Monitor(), find_keys()       
        #return (self.spec_general_data, self.B_list, self.L_list, self.for_json_sta)
