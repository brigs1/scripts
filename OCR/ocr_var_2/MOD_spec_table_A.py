...
class ComBld():
    ...
    def spec_table_A(self):
        ...
        self.B_list = [] #building_attr을 B_list로 넣기 위해 spec_general_data 를 B_list로 치환
        self.L_list = []
        #self.spec_general_data = [] # spec_general_data 를 B_list로 치환
    ...
    #class CoBldSpecGen():
            ...
            if len(no2_landclass_level) > 0:   #'지목'항목에 무언가 있다면
                for vg in  num_checked_coord_box:  # 숫자까지 체크된 좌표텍스트계에서,
                    ...
                    if addr_L < midpoint(vg)[0] < addr_R and memo_Y + 50 < midpoint(vg)[1] < no2_landclass_level[0]:
                        ...
                        self.building_attr["소재지"] = "{}".format(vg[5])  # 빌딩 속성이란 딕트에 "소재지"로도 넣음... 이건 뭐지


                        if re.search("^[가-힣]+\d*도$", vg[5]):   
                            self.building_attr["도"] = "{}".format(vg[5])

                        elif re.search("^[가-힣]+\d*시$", vg[5]):   
                            self.building_attr["시"] = "{}".format(vg[5])
                            ...
                            self.building_attr["면"] = "{}".format(vg[5])
            ...
            if len(b_name_1) > 0:
                ...
                self.building_attr["건물명_1"] = ' '.join(new_b_name_1)
                self.for_json_sta["건물명_1"] = ' '.join(new_b_name_1)
                        
            if len(b_name_2) > 0:
                new_b_name_2 = []
                b_name_2.sort(key=lambda t: t[0][1])
                self.building_attr["건물명_2"] = ' '.join(new_b_name_2)
                ...
                if len(groups) > 0:
                    ...
                    self.building_attr["건물명_bn"] = ' '.join(new_bn_2)        
            ...
            if len(use_mode) > 0:
                ...
                self.building_attr["이용상황"] = ' '.join(new_use_mode)
            ...
            if len(addr) > 0:
                ...
                self.building_attr["소재지"] = ' '.join(new_addr)
            ...
            if len(addrnum) > 0:
                ...
                for nadm in addrnum:
                    ...
                    #if 600 < adm[0][1] < no2_landclass_level[0]:
                self.building_attr["지번"] = ' '.join(new_addrnum)
            ...
            if len(floor_num) > 0:            
                ...
                self.building_attr["지상 층 수"] = int(floor_num[-1])
                self.for_json_sta["지상 층 수"] = int(floor_num[-1])


            if len(self.building_attr) > 0:
                #print("\n", "building_attr", self.building_attr, "\n")

                self.B_list.append(self.building_attr)
            ...
            #print("\n\n\n\", "정리된 내용 한번 봅시다----------------", "\n\n\n\")   
        #print("spec_general_data:::",self.spec_general_data, "\n")
        print("B_List:::", self.B_list, "\n")
        print("L_list:::",self.L_list, "\n")
        #print("for_json_sta:::",self.for_json_sta, "\n")




        return (self.B_list, self.L_list, self.for_json_sta)
...
#                     building_attr["소재지"] = "{}".format(vg[5])  # 빌딩 속성이란 딕트에 "소재지"로도 넣음... 이건 뭐지


#                     if re.search("^[가-힣]+\d*도$", vg[5]):   
#                        building_attr["도"] = "{}".format(vg[5])

#                     elif re.search("^[가-힣]+\d*시$", vg[5]):   
#                        building_attr["시"] = "{}".format(vg[5])
                        
#                     elif re.search("^[가-힣]+\d*[군구]$", vg[5]):
#                        building_attr["군구"] = "{}".format(vg[5])                        

#                     elif re.search("^[가-힣]+\d*[동]$", vg[5]):
#                        building_attr["동"] = "{}".format(vg[5])

#                     elif re.search("^[가-힣]+\d*[면]$", vg[5]):
#                        building_attr["면"] = "{}".format(vg[5])

#                     elif re.search("^[가-힣]+\d*[리]$", vg[5]):
#                        building_attr["리"] = "{}".format(vg[5])
...
#                 elif landclass_R < midpoint(vg)[0] < paper_L:
#                     if re.search("\w+조$|철근\w+조$|콘크\w+조$|크리\w+조$|세멘\w+조$|시멘\w+조$|시맨\w+조$", vg[5]):   
#                         building_attr["구조"] = "{}".format(vg[5])
#                         for_json_sta["구조"] = "{}".format(vg[5])

#                     elif re.search("\w+붕|슬라브\w+붕$|경사\w+붕$", vg[5]):   
#                         building_attr["지붕"] = "{}".format(vg[5])
...
#                     new_b_name_1.append(bnm1[5])
#                 #if 600 < bnm[0][1] < no2_landclass_level[0]:
#             building_attr["건물명_1"] = ' '.join(new_b_name_1)
#             for_json_sta["건물명_1"] = ' '.join(new_b_name_1)
                    
#         if len(b_name_2) > 0:
#             new_b_name_2 = []
#             b_name_2.sort(key=lambda t: t[0][1])
#             building_attr["건물명_2"] = ' '.join(new_b_name_2)
...
#                     if nadm[5] not in new_bn_2:
#                         new_bn_2.append(nadm[5])
#                 building_attr["건물명_bn"] = ' '.join(new_bn_2)        
...
#                 if adm not in new_use_mode:
#                     new_use_mode.append(adm)
#             building_attr["이용상황"] = ' '.join(new_use_mode)
...
#                     new_addr.append(adm[5])
#                 #if 600 < adm[0][1] < no2_landclass_level[0]:
#             building_attr["소재지"] = ' '.join(new_addr)
...
#                     new_addrnum.append(nadm[5])
#                 #if 600 < adm[0][1] < no2_landclass_level[0]:
#             building_attr["지번"] = ' '.join(new_addrnum)
...
#         if len(floor_num) > 0:            
#             floor_num.sort(key=int) # 층수는 string이므로 이와 같이 "정수기준이라면"으로 정렬후 가장 큰 마지막 숫자 선택
#             building_attr["지상 층 수"] = int(floor_num[-1])
#             for_json_sta["지상 층 수"] = int(floor_num[-1])


#         if len(building_attr) > 0:
#             print("\n", "일반항목", building_attr, "\n")

#             spec_general_data.append(building_attr)
...
#                                 elif "지번" not in building_attr:
#                                     building_attr["지번"] = "{}".format(va[5])
#                                     for_json_sta["지번"] = "{}".format(va[5]) 
#                                 #print(integral_attr)
#                                 #print(integral_attr, "general에 지번 들어감")
...
#         #print("\n\n\n\", "정리된 내용 한번 봅시다----------------", "\n\n\n\")   
#     print(spec_general_data, B_list, L_list, for_json_sta)
